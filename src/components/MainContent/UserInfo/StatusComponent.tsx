...
type Props = {
  ...
  userStatusData?: string | null;
  authId: number;
  userId: number;
  placeholder: string;
  className?: string;
  children?: any;
  userInfoPropertie?: string;
  setInfoState?: (data: UserInfoType) => void;
  setStatusState?: (data: string) => void;
};

const StatusComponent: React.FC<Props> = (props): JSX.Element => {
  const userInfo = props.userInfo;

  const [editMode, setEditMode] = useState(false);
  const [hoverMode, setHoverMode] = useState(false);
  const [statusInput, setStatusInput] = useState<string>(props.currentData!);
  ...
  const setProfileStatus = () => {
    props.setStatusState!(userStatusInput!);
    setEditMode(false);
  };
